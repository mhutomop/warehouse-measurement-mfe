...
import { deleteMeasurement } from "../apis/measurements.api";
...
class DeleteMeasurement extends React.Component {
  ...
  async handleSubmitButtonClicked() {
    try {
      let response = await deleteMeasurement(this.props.item.id);
      ...
      <React.Fragment>
        {
          ...
          <div className="p-6 space-y-6">
            <form className="w-full text-base">
              <label>Are you sure to delete this measurement [{this.props.item.unit}] ?</label>
            </form>
          </div>
